package ios;
//## Signup flow
//Verify new customers can be signed up from uber marketplace
//To verify we can able click - "LOGIN WITH UBER" page
...
import pages.UberMarketplace;

public class Signup_VehicleMarketPlace extends BaseClass {
	...
	//initialise null objects for pages files
		...
		UberMarketplace uberpg;
	
		@Description("FordDrive uber marketplace signup begin")
		@Epic("Uber Marketplace Page")
		@Feature("Feature2: Uber Marketplace Page sigbup")
		@Severity(SeverityLevel.CRITICAL)
		@Test
	@SuppressWarnings("deprecation")
	    public static void SignupForddriveMarketplace() throws MalformedURLException {
		...
		UberMarketplace uberpg = new UberMarketplace(driver);
		
		// run launch chrome
		try {

			// fill details on Partial signup
			FileInputStream fs = new FileInputStream("/Users/sgrover7/eclipse-workspace/ios/src/test/resources/InputFile/Signup_VehicleMarketPlace.xlsx");
			...
			for (int j = 1; j <= rowNum; j++) {
				...
				for (int i = 1; i <= noOfsignups; i++) {
					...
					System.out.println(context);
						...
						driver.get("https://preprod.rideshare.forddrive.com/sign-up?");
						String context=((SupportsContextSwitching) driver).getContext();
						System.out.println(context);
						
					 } else if (env.contains("dev")) {
						 driver.get("https://dev.rideshare.forddrive.com/sign-up?");
							String context=((SupportsContextSwitching) driver).getContext();
							System.out.println(context);
							
					 } else {
						 driver.get("https://rideshare.forddrive.com/sign-up?");
						String context=((SupportsContextSwitching) driver).getContext();
						System.out.println(context);
					}
					...
					//click on login with uber button
					uberpg.Button_LoginWithUber.click();
					Thread.sleep(5000);	
					
					//allowwpopup -- try catch
					uberpg.allow_uber_popup();
					
					//click on person icon
					uberpg.uber_right_corner();
					
					//enter email
					uberpg.text_uberEmail.click();
					uberpg.text_uberEmail.sendKeys("andrew.brown+test+bell@uber.com");
					
					//click on continue button
					uberpg.button_continueEmail.click();
					
					//try catch for passowrd or otp
					uberpg.handle_password();
					 
					//allowwpopup -- try catch
					uberpg.allow_uber_popup();
					
					//forddrive tile click
					uberpg.icon_forddrivetile.click();
					Thread.sleep(3000);
					
					//get started click
					uberpg.button_uberGetStarted.click();
					Thread.sleep(6000);
						
					//change to stage link
					...
					if (tier.contains("1")) {
						pkgpg.pkg_tier1.click(); // chnage tier
					} else if (tier.contains("2")) {
						pkgpg.pkg_tier2.click();
					} else  {
						pkgpg.pkg_tier3.click();
					}
					Thread.sleep(5000);
		...
		@AfterMethod
		@Description("uber marketplace Signup page process end")
		@AfterTest
