...
public class PackagesPage extends BaseClass {
	...
	@iOSXCUITFindBy (xpath = "//XCUIElementTypeStaticText[@name=\"289\"]")
	public WebElement tiervalue_SD1;
	
	@iOSXCUITFindBy (xpath = "//XCUIElementTypeStaticText[@name=\"329\"]")
	...
	@iOSXCUITFindBy (xpath = "//XCUIElementTypeStaticText[@name=\"349\"]")
	...
	public void verify_SD() {
		...
		if(tier2SD) {
			boolean milesADV=miles_advanced.isDisplayed();
			if(milesADV) {
		...
		boolean tier3SD=tiervalue_SD3.isDisplayed();
		if(tier3SD) {
			boolean milesprem=miles_premium.isDisplayed();
			if(milesprem) {
			Assert.assertTrue(true, "tier 3 miles in SD is correct");
			}
			else {
			...
			Assert.assertTrue(false, "tier 3 value  and miles in SD is incorrect");
		}
	}
	...
	public void verify_LA() {
		...
		boolean tier3LA=tiervalue_LA3.isDisplayed();
		if(tier3LA) {
			boolean milesprem=miles_premium.isDisplayed();
			if(milesprem) {
			Assert.assertTrue(true, "tier 3 miles in LA is correct");
			}
			else {
			...
			Assert.assertTrue(false, "tier 3 value  and miles in LA is incorrect");
		}
	}
